<template>
    <transition name="slide-panel">
       ...
       <div class="flex justify-between items-center mb-4">
          ...
          <div class="p-3 bg-gray-50 rounded-lg">
             <div class="flex items-center">
                ...
                <div class="flex-1 mx-[5px] text-center text-sm font-medium text-gray-700">
          ...
          <button @click="handleToggleSlideList" class="text-gray-500 hover:text-gray-700 cursor-pointer">
       ...
       <ul class="space-y-2 overflow-y-auto h-[calc(100%-140px)]">
          <li v-for="(element, index) in imageList" :key="index" @click="handleSelectImage(index)"
             :class="['p-3 cursor-pointer rounded-lg transition duration-200 border', currentIndex === index ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50']">
             <div class="flex items-center">
                <div class="flex-shrink-0 w-12 h-12 bg-gray-200 rounded-md overflow-hidden mr-3">
                   <img :src="element.url" :alt="element.slideName" class="w-full h-full object-cover" />
                </div>
                <div class="flex-1 min-w-0">
                   <div class="font-medium text-gray-800 truncate">{{ element.slideName }}</div>
                   <div class="text-sm text-gray-600">{{ element.adoptedPart }}</div>
                </div>
             </div>
          </li>
       </ul>
